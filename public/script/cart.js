...
function appendData(){
   data.forEach((el)=>{
        if(!el.Product_Type){
            el.Product_Type="Item";
        }
        if(!el.Product_Id){
            el.Product_Id= el["Product Id"]
        }
        var div= create("div");
        div.class="bag one";
        div.id="appendcont"
        ...
        <div>
            ...
            <button style="margin-left:20%; " id=${el.Product_Id} class="remove"><img style="margin-left:-3436%" title="Remove item" src="https://cdn-icons-png.flaticon.com/512/3964/3964013.png" height=20px alt="Remove"></button>
            </div>
...
function remove(el){
    ...
    my_prod_for_del.forEach((pro) => {
        ...
        if (pro.Product_Id === el || pro["Product Id"]===el) {
